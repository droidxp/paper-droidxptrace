...
of the \mas to correctly classify a sample as a malware. \fh{Both versions (original/repackaged) from the \gps family have the same behavior of present ads, however the repackaged version have more call points to Ads API, and Ads sources different. This behavior is not so harmful when compared to others repackaged apps, however nothing prevents others malware families to also download apks at runtime, and execute additional calls to sensitive APIs different from the original version.}
