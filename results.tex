...
\fh{Our analyses show that both app version, original and repackaged from the \gps family, have the same behavior of present ads. However, from repackaged version, we can find more call points to Ads API than its respective original version, and from different sources. This behavior is not so harmful when compared to other repackaged apps that can leak sensitive information. However, nothing prevents others malware families to also download apks at runtime, and execute additional calls to sensitive APIs different from the original version.}
