...
\fh{here we must review logistic regression}Here we hypothesize that \sscore might have an
...
that the \mas \fh{maybe this result will change} is more likely to assign a correct label to
...
of this sample of 30 \gps malware reveals a few modification patterns when comparing the original and the
...
In most of the cases, the repackaged version just changes the Manifest file to modify the package name or name of the main
activity. Moreover, 29 out of the 30 samples in this dataset  {\bf modifies} the 
...
be also an strategy for antivirus evasion. For instance, although
