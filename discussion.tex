...
\review{
\item \textbf{Malware Family Analysis (RQ3).} The results indicate that a some families are responsible for the largest number of false negatives in the complete dataset. We specifically investigated one malware family described as \gps, which corresponds to a particular type of
Adware, designed to automatically display advertisements while an app is running. After reverse engineering a sample of 30 \gps malware, we confirmed that the \mas cannot identify the patterns of changes introduced in the repackaged versions of the apps. The prevalence of the \gps family in the \cds contributed to explaining the poor performance of the \mas for malware classification in the large dataset. }
