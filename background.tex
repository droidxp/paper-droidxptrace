...
\subsection{Study characterization}

Based on the taxonomy from Juristo and Gómes~\cite{DBLP:journals/ese/CarverJBV14}, we can characterize our study as follows:

\begin{enumerate}[1]
 \item \textbf{Objective}:  Our replication study over the \sds has the "purpose of operational replication" since it "checks that the experimental recipe produces the same results with another researcher"---a necessary step to confirm that our procedures are similar to the original study. Differently, the replication study using the \cds aims at an "empirical generalization", since it "tests the extent to which the [original] study results are generalizable to other populations".

\item \textbf{Similarity with the previous study:} Our work corresponds to an independent replication because (a) the works involve different researchers, (b) we use a different sample of malware in the \cds, and we use additional methods for data analysis (including accuracy metrics and triangulation with VirusTotal for labeling the samples in our study).
