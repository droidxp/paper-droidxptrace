...
However, we encountered compatibility issues with many samples, either in relation to DroidFax~\cite{DBLP:conf/icsm/CaiR17a} or the versions of the Android emulator we employed (API level 28). First, out of 2,776 original apps, DroidFax instrumented 1,990 samples, resulting in a total of 4,958 app pairs (original/repackaged). Among these 4,958 app pairs, 4,668 samples from repackaged pair could be successfully instrumented by DroidFax. Finally, out of the remaining 4,668 app pairs, 841 pairs failed to install due to Android emulator compatibility issues or other emulator settings. Consequently, after removing incompatible app pairs, we selected 3,827 app pairs (original/repackaged) from \repack.
...
\review{From these app pairs, we had 507 unique originals, of which DroidFax instrumented 374 samples, resulting in a total of 917 app pairs. From these pairs, DroidFax instrumented 777 samples from repackaged pair. Finally, we eliminated 6 pairs that showed incompatible issues with the versions of the Android emulator used in our research (API level 28). In total, we selected 771 app pairs from \amc to curate our dataset. It is important to highlight that at both datasets, we had lower initial sample loss rates when compared to the dataset from original study~\cite{DBLP:conf/wcre/BaoLL18}.}
