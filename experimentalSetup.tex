...
\fh{Lastly, we also explored all sensitive APIs called by all app pairs at our dataset. This brought to the light the most frequently abused sensitive APIs that appear in the repackaged, malicious version of our samples. We observed that when executed all sample from \cds, malicous app version injected $75$ sensitive APIs distinct at least one time (from our list of 261 sensitive APIs). Compared to Previous studies~\cite{DBLP:conf/wcre/BaoLL18,DBLP:journals/jss/CostaMMSSBNR22}, the samples from our dataset has a wider range of sensitive APIs injected into its malicious version. Table XXX presents the list of the most frequent sensitive APIs injected at malicious version from our dataset.}
