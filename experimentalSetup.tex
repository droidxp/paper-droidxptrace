...
two \ses identify a malicious behavior within the asset. This is in accordance with previous research~\cite{vt-label,DBLP:journals/ese/KhanmohammadiEH19}. Conversely, considering the \cds, at least two \se identified 662 out of the \apps repackaged apps as malware (54.30\%).
...
of them from the Gappusin (37.00\%), Revmob (20.69\%), and Dowgin (10.87\%) families.
...
\item \textbf{Data Collection}: During the execution of the instrumented app, we collect all relevant information (such as calls to sensitive APIs, test coverage metrics, and so on). We use this information to analyse the performance of the \mas for detecting malicious behavior.
\end{enumerate}
...
if the approach was able or not to identify the malicious behavior. We also use existing tools to reverse engineer a sample of repackaged
