...
\fh{Lastly, we also explored all sensitive APIs called by samples from our dataset. This brought to the light the most frequently abused sensitive APIs that appear in the repackaged, malicious version of our samples. We observed that when executed all samples from \cds, malicious app version injected $138$ sensitive APIs distinct at least one time (from our list of 261 sensitive APIs), whereas Previous studies~\cite{DBLP:conf/wcre/BaoLL18,DBLP:journals/jss/CostaMMSSBNR22} got the most of 131 inserts. The sensitive API that is abused the most by repackaged apps from our dataset is \textbf{java.lang.reflect.Field: java.lang.Object get(java.lang.Object)}. Reflection is an API that can be used to modify the behavior of methods, classes, and interfaces at runtime. Malicious code frequently may use these APIs, or other to help in its attempt to compromise security in the system. Table~\ref{tab:APIused} presents the list of the most frequent sensitive APIs injected at malicious version from our dataset.}
