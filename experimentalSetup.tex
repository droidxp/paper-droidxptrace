...
We use two repositories of repackaged Android apps (\repack and \amc) to curate the
...
apps—--many repackaged versions of the same original app may coexist within the \repack dataset. Although \repack is the leading dataset used in Android
repackaged research, it only contains packages built up until 2018. For this reason, we decided to include samples from the \amc
dataset that were built after 2018 in our research. However, unlike RePack, \amc lacks information about the original apps, leading
us to follow an existing algorithm~\cite{~\cite{DBLP:journals/tse/LiBK21}} to identify the original versions of repackaged apps in \amc.
We then built a sample from the \amc dataset that contains 1,190 pairs (original/repackaged) of apps—--all pairs
satisfying our constraint of being built after 2018. Altogether, our initial dataset contains a total of 16,687 pairs of apps.

During the execution of our experiments, we encountered a small number of issues with many samples.
...
Initially, we encountered failures while instrumenting 919 original apps from our initial dataset, which comprised 3,283 original apps collected from \repack and \amc.
After removing these original apps from our dataset, we were left with a total of 5,875 app pairs (original/repackaged). Among these pairs,
430 repackaged apps could not be instrumented, resulting in a total of 5,445 pairs of apps.
Out of the remaining 5,445 pairs that we successfully instrumented using DroidFax, 847 pairs failed to execute in the Android emulator.
To sum up, after filtering out apps that either failed to instrument or execute, we curated a dataset containing a total of 4,598
app pairs (original/repackaged). Compared to our experience building our dataset, a larger percentage of similar instrumentation and execution issues have been reported in previous research~\cite{DBLP:conf/wcre/BaoLL18}.

Finally, we queried the \vt repository
...
do not satisfy this constraint. In the end, we are left with our \cds of \apps apps which we use in our study. 

To bring evidence that we were able to reproduce the results of previous research, we also consider in our research
a small dataset (hereafter \sds)
