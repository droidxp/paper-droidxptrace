...
\todo[inline]{Looks like a naive approach. Note that, malware developers often use advanced techniques for hinder security experts to detect the presence of a malicious change in a app---using both static or dynamic analysis. The approach detailed here might be efficient only for poor designed malware. I am discussing this because we are seriously considering submiting this paper to a security venue. The people involved in Usenix Security are dealing with more advanced situations, like malare that changes its behavior when it detects that it is running in um emulator, for instance.}
