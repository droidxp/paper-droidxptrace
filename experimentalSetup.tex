...
Secondly, during the execution of our experiments, we encountered recurrent issues with many samples in our initial dataset. Many of these issues arose during the process of instrumenting the apps using DroidFax~\cite{DBLP:conf/icsm/CaiR17a}. Other problems occurred during the installation of apps or while formulating reports from the execution of apps in the Android emulator. More specifically, we encountered failures while instrumenting 919 original apps from our initial dataset, including both \repack and \amc. After removing these original apps from our dataset, we were left with a total of 5,875 pairs (original/repackaged) of apps. Among these pairs, 430 repackaged apps could not be instrumented. Additional failures occurred while installing or synthesizing information arising from app execution (either the original or the repackaged version), resulting in a dataset containing a total of 4,802 pairs. Compared to our experience building our dataset, a larger percentage of samples failures have been reported in previous research~\cite{DBLP:conf/wcre/BaoLL18}.
...
Our \cds has an average similarity score of \review{(90.39)\%}---with the follow distribution \review{($87$ of
