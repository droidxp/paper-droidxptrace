...
with users using different app stores.  The focus of this paper is the Mining Android Sandbox, hereafter \mas which has been shown as the most effective
...
But these previous studies have two main limitations. \fh{
First, they use a small dataset of malware comprising only \appsSmall pairs of original/repackaged versions of an app. Second, their impact analysis is not fine-grained enough; crucially, they do not analyse the impact of malware characteristics on accuracy, e.g., the similarity index between the original app and the corresponding repackaged malware, and the repackaged malware family (gappusin, kuguo, dowgin, etc.).}
...
\fh {First, we check if there is an association between the lower accuracy and the difference (or similarity) in the packages of app pairs (original/repackaged versions), used at both dataset (previous studies and our studies). Our results show that Similarity Score~\cite{DBLP:journals/tifs/0029LBKTLC17} does not influence the lower accuracy in the larger dataset. However, when observing the family samples, our experiments show that the \mas fail to correct classify the most of samples
from the {\color{red}\gps malware family} (a particular class of adware that frequently appears in repackaged apps). Out of the total of \appsGps samples within this family, the \mas failed to correctly classify \appsGpsFN samples as malware (false positive), worst accuracy when compared with others malware family with expressive samples at our dataset. Our results reveal that this particular family is
